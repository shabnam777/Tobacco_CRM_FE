...
class ApiService {
  ...
  static Future<dynamic> get(String path, {Map<String, String>? params}) async {
    ...
    catch (e) { throw ApiException('Network error: $e'); }
  }
  ...
  static dynamic _handle(http.Response res) {
    ...
    if (res.statusCode == 401) { StorageService.clearToken(); throw ApiException('Unauthorized', statusCode: 401); }
  ...
  static Future<dynamic> addNote(String id, String note) => post('/leads/$id/notes', body: {'note': note, 'timestamp': DateTime.now().toIso8601String()});

  // Email
  static Future<dynamic> sendEmail(Map<String, dynamic> p) => post('/email/send', body: {
    ...p, 'sender_email': StorageService.getSenderEmail(), 'sender_name': StorageService.getSenderName()});
  static Future<List<dynamic>> getEmailHistory(String id) =>
      get('/email/history/$id').then((d) => d is List ? d : <dynamic>[]);

  // AI
  static Future<dynamic> generateEmail(Map<String, dynamic> p) => post('/ai/generate-email', body: {
    ...p, 'groq_key': StorageService.getGroqKey(),
    'sender_name': StorageService.getSenderName(), 'sender_company': StorageService.getCompanyName()});
  static Future<dynamic> generateFollowup(Map<String, dynamic> p) => post('/ai/generate-followup', body: {
    ...p, 'groq_key': StorageService.getGroqKey()});

  // Discovery
  static Future<dynamic> discoverImporters() => post('/discovery/search', body: {
    'groqKey': StorageService.getGroqKey(), 'cfToken': StorageService.getCfToken(),
    'cfAccountId': StorageService.getCfAccountId(),
    'count': 10, 'runAnalysis': true});
  ...
  static Future<dynamic> revalidateDiscovery(String id) => post('/discovery/validate/$id', body: {
    'groqKey': StorageService.getGroqKey(), 'cfToken': StorageService.getCfToken(),
    'cfAccountId': StorageService.getCfAccountId()});
  ...
  static Future<List<dynamic>> getCampaigns() =>
      get('/campaigns').then((d) => d is List ? d : <dynamic>[]);
