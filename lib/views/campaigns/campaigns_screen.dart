...
class _CampaignsScreenState extends ConsumerState<CampaignsScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => ref.read(campaignProvider.notifier).loadCampaigns());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          actions: [
            IconButton(
                icon: const Icon(Icons.refresh, color: AppColors.textMuted), onPressed: () => ref.read(campaignProvider.notifier).loadCampaigns())
          ]),
      body: RefreshIndicator(
          ...
          child: ListView(padding: const EdgeInsets.all(16), children: [
            ...
            _mondayCard(),
            const SizedBox(height: 16),
            _header('Campaign History', camp.campaigns.length),
  ...
  Widget _stats(CampaignState c) {
    ...
    return Row(children: [
      ...
      _box('$sent', 'Sent', AppColors.accentGold),
      const SizedBox(width: 10),
      _box(sent > 0 ? '${(opened / sent * 100).toStringAsFixed(0)}%' : '0%', 'Open', AppColors.accentTeal),
      const SizedBox(width: 10),
      _box(sent > 0 ? '${(replied / sent * 100).toStringAsFixed(0)}%' : '0%', 'Reply', AppColors.success),
    ]);
  }

  Widget _box(String v, String l, Color c) => Expanded(
      child: Container(
  ...
  Widget _mondayCard() {
    ...
    return Container(
        ...
        child: Row(children: [
          ...
          Switch(value: true, onChanged: (_) {}, activeThumbColor: Colors.white, activeTrackColor: Colors.white38),
        ]));
  }

  Widget _header(String t, int n) => Padding(
  ...
  Widget _empty() => Padding(
      ...
      child: Column(children: [
        const Icon(Icons.campaign_outlined, size: 44, color: AppColors.textDisabled),
...
class _CampaignCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = campaign.status == 'Completed'
        ? AppColors.success
